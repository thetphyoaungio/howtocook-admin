...
export class AllUsersUserDetailComponent implements OnInit, OnDestroy {
    userId:any;
    userDetail:any;
            ...
            next:((res:any) => {
                ...
                this.userPosts = [...(res.data.map((x:any) => 
                    ({
                        id: x.id,
                        title: x.title,
                        createdAt: x.createdAt && this.datePipe.transform(this.dateTimeService.createDateForSafariMac_5(x.createdAt), 'dd MMM yyyy, hh:mma'),
    ...
    gotoPostDetail(post:any) {
        this.router.navigate([`/${RouteNames.DASHBOARD}/${RouteNames.USERS}/${RouteNames.ALL_USERS}/${RouteNames.ALL_USERS_POST_DETAIL}`, 
        post.id, '']);
    }
