...
export class RequestChefUsersHomeComponent implements OnInit, OnDestroy {
    ...
    getAllUsers() {
        ...
        this.userService.getRequestedChefUsers(qry).subscribe({
            next:((res:any) => {
                ...
                this.allusers = [...(res.data.map((x:any) => {
                    return {
                        ...
                        requestedStatus: x.requestedAt ? 'Requested' : 'Accepted',
                ...
                this.toid1 = setTimeout(() => {
                    this.tableData = {
                        title: MenuLabels.REQUEST_CHEF_USERS,
    ...
    gotoDetail(user:any) {
        this.router.navigate([`/${RouteNames.DASHBOARD}/${RouteNames.USERS}/${RouteNames.REQUEST_CHEF_USERS}/${RouteNames.REQUEST_CHEF_USERS_USER_DETAIL}`, user.id]);
    }
