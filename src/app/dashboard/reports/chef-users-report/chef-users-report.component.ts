...
@Component({
    templateUrl:'./chef-users-report.component.html',
    styleUrls:['./chef-users-report.component.scss']
})
export class ChefUsersReportComponent implements OnInit, OnDestroy {
    reports:any;
    ...
    getReports() {
        ...
        let qry = `?search=${this.searchTxt||''}&page=${this.p}&limit=${this.perPage}&startDate=${this.startDate||''}&endDate=${this.endDate||''}`;

        this.expQuery$ = `?search=${this.searchTxt||''}&startDate=${this.startDate||''}&endDate=${this.endDate||''}`;

        this.reportService.getChefUsersReport(qry).subscribe({
            next:((res:any) => {
                ...
                if(res.data) {
                    this.reports = [...(res.data.map((x:any)=>({
                        ...
                        approvedBy: x.approvedUser?.userName || '-',
                    })))];
                }
